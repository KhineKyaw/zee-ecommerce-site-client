    ...
    return (
        <div className={classes.category}>
            ...
            <div className={classes.option_container}>
                ...
                <Card key={option.value} className={`${classes.option} ${optionIndex === selectedOption && classes.selected}`} onClick={() => handleOptionClick(optionIndex)}>
                    {option.image ? (
                    <img className={classes.option_image} src={option.image} alt={option.value}></img>
                    ) : (
                    option.value[0]
                    )}
                </Card>
